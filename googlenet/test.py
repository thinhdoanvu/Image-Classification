...
if __name__ == "__main__":
    ...
    clf = models.googlenet(weights='IMAGENET1K_V1').to(device)# or weights='IMAGENET1K_V1', weights=None
    num_features = clf.fc.in_features
    clf.fc = torch.nn.Linear(num_features, num_classes)
    clf.to(device)

    folder_checkpoints = 'checkpoints'  # Define the folder name
    file_checkpoints = os.path.join(folder_checkpoints,'googlenet_epoch_23_acc_0.9940.pt')#lay epoch cuoi cung
    clf.load_state_dict(torch.load(file_checkpoints, device))
