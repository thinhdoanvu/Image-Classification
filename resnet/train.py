...
def visualize_and_save_pretrained_weights(model, epoch):
    ...
    for i, ax in enumerate(axes.flatten()):
        ...
        ax.axis('off')
    folder_path = 'visualize_pretrainweight'  # Define the folder name
...
def validate_model():
    ...
    with torch.no_grad():
        ...
        return valid_loss / len(valid_loader)  # hàm trả về loss trung bình trên 1 epoch (tích lũy từ các batch)
...
if __name__ == "__main__":
    ...
    # Using 2.1 or 2.2
    ##########################
    # 2.1 Fine-tune for training
    import torchvision.models as models

    clf = models.resnet18(weights=None)  # or weights='IMAGENET1K_V1'

    # Note that the model pre-trained model has 1,000 output neurons (because ImageNet has 1,000 classes), so we must
    # customize the last linear layer to adapt to our 53-class problem
    num_class = 53
    num_features = clf.fc.in_features
    clf.fc = torch.nn.Linear(num_features, num_class)
    clf.to(device)

    # 2.2 Or trainning from Scratch
    # import torchvision.models as models
    #
    ...
    for epoch in range(n_epoch):  # train for n_epoch
        ...
        if valid_err < max_err:
            file_path = os.path.join(folder_path,
                                     'resnet18_epoch_' + str(epoch) + '_valoss' + '_{0:.4f}'.format(valid_err) + '.pt')
