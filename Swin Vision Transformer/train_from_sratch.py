...
if __name__ == "__main__":
    ...
    # 1. Load the dataset
    transform = transforms.Compose([Resize((224, 224)), ToTensor()])                                                    #Model for swin_T require 224x224
    ...
    #model = timm.create_model('swin_tiny_patch4_window7_224', in_chans=3, pretrained=True, num_classes=num_classes).to(device)
    ...
    for epoch in range(100):
        ...
        if val_acc > max_acc:
            ...
            file_path = os.path.join(folder_path,'svit_epoch_' + str(epoch) + '_acc_{0:.4f}'.format(max_acc) + '.pt')
