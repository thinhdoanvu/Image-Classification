...
from utils.models import LeNetBackbone
...
def main():
    ...
    # ======== 3. Khởi tạo mô hình ========
    model = LeNetBackbone(input_size=INPUT_SIZE).to(DEVICE)
    criterion = ContrastiveLoss(margin=MARGIN)
    optimizer = Adam(model.parameters(), lr=LEARNING_RATE)

    best_val_loss = float('inf')
    epochs_no_improve = 0
    model_save_path = "weights/best_fit.pt"
    ...
    graph(train_losses, val_losses, accuracies, precisions, recalls, save_path="training_plot.png")
