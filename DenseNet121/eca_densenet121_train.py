...
def plot_metrics(train_losses, val_losses, train_accuracies, val_accuracies):
    ...
    plt.savefig("eca_densenet121_trainplot.png")  # You can change the file name and format (e.g., .png, .jpg, .pdf)
...
if __name__ == "__main__":
    ...
    from utils.models import DenseNet121_ECA
    ...
    for epoch in range(80):
        ...
        if val_acc > max_acc:
            ...
            folder_path = 'checkpoints_DenseNet121_ECA'  # Define the folder name
            if not os.path.exists(folder_path):
                os.makedirs(folder_path)  # Create the folder if it does not exist
            file_path = os.path.join(folder_path,
                                     'DenseNet121_ECA_epoch_' + str(epoch) + '_acc_{0:.4f}'.format(val_acc) + '.pt')
