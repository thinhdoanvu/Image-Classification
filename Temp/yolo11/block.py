# C:\Users\csipl\anaconda3\envs\yolo11\Lib\site-packages\ultralytics\nn\modules\block.py
"""
use for ultralytics ver: 8.3.2
30 Sept 2024
Thinhdv
"""
...
__all__ = (
    "DFL","HGBlock","HGStem","SPP","SPPF","C1", "C2","C3","C2f","C2fAttn","ImagePoolingAttn","ContrastiveHead","BNContrastiveHead","C3x",
    "C3TR","C3Ghost","GhostBottleneck","Bottleneck","BottleneckCSP","Proto","RepC3","ResNetLayer","RepNCSPELAN4",
    "ELAN1","ADown","AConv","SPPELAN","CBFuse","CBLinear","C3k2","C2fPSA","C2PSA","RepVGGDW","CIB","C2fCIB","Attention","PSA","SCDown",
    ...
    "Spatial_Attention", "CBS", "ScaleDotProduct", "Contigous_Att", "CBSE"
)
...
class CBS(nn.Module): # Rename to CBRE (Convol BatchNorm ReLu ECA) + Down Dimension
    # Standard convolution with args(ch_in, ch_out, kernel, stride, padding, activation)
    # output = input/2
    default_act = nn.ReLU()
    ...
    def __init__(self, c1, c2):
        super().__init__()
        self.conv1 = nn.Conv2d(in_channels=c1, out_channels=c2, kernel_size=(3, 3), stride=(2, 2), padding=(1, 1))
        self.batchnorm1 = nn.BatchNorm2d(c2)
        self.act = self.default_act
        self.conv2 = nn.Conv2d(in_channels=c2, out_channels=c2, kernel_size=(3, 3), stride=(1, 1), padding=(1, 1))
    ...
    def forward(self, x):
        ...
        x = self.batchnorm1(x)
        x = self.act(x)
        # x = self.eca(x) # ver2 (lan 2) - train 12: 68.2 %
        x = self.conv2(x)
        x = self.batchnorm2(x)
        x = self.eca(x)   # ver2(lan 1) - train 11: 68.5%
        #print("# CBS Output Size: ", x.size())
...
class Spatial_Attention(nn.Module):    # Input CxHxW
    def __init__(self, c1, c2):
        ...
        self.ha = H_Attention(c1, c1)
        self.wa = W_Attention(c1, c1)
        self.mp5 = nn.MaxPool2d(kernel_size=5, stride=1, padding=5//2)
        self.mp9 = nn.MaxPool2d(kernel_size=9, stride=1, padding=9//2)
        self.mp13 = nn.MaxPool2d(kernel_size=13, stride=1, padding=13//2)
    ...
    def forward(self, x):
        ...
        mpl5 = self.mp5(x)
        # print("# MP5 Output Size: ", mpl5.size())           # B x C1 x H x W
        mpl9 = self.mp9(mpl5)  
        # print("# MP9 Output Size: ", mpl9.size())           # B x C1 x H x W
        mpl13 = self.mp13(mpl9)    
        # print("# MP13 Output Size: ", mpl13.size())         # B x C1 x H x W
        
        cat = torch.cat((x,mpl5,mpl9,mpl13),dim=1)
        # print("# CAT Output Size: ", cat.size())            # B x 4*C1 x H x W
        con = self.conv(cat)
        # print("# CONV Output Size: ", con.size())           # B x C2 x H x W
        spa_att = self.sl(self.bn(con))
        # print("# SPD Output Size: ", spa_att.size())      # B x C2 x H x W
        return spa_att
...
class ScaleDotProduct(nn.Module):    # Input CxHxW
    ...
    def forward(self, x):
        ...
        # print("# Height Output Size: ", h_att.size())       # B x C1 x H x 1
        w_att = self.wa(x)                                  
        # print("# Width Output Size: ", w_att.size())       # B x C1 x 1 x W
        c_att = self.ca(x)                                  
        ...
        attention_hw = F.softmax(scores_scaled, dim=-1)  # Shape: [Batch, Channels, Channels]
        # print("# Attention height-width Output Size: ", attention_hw.size())     # B x C1 x C1

        # Multiply the attention width with the value vector V
        BV,CV,HV,WV = c_att.size()
        V = c_att.view(BV,CV,HV*WV)
        
        output = torch.bmm(attention_hw, V)    # Shape: [Batch, Channels, 1]
...
class CBSE(nn.Module):    # Input CxHxW
    def __init__(self, c1, c2):
        super().__init__()
        self.conv = nn.Conv2d(c1, c2, 1, 1)   # after concate 3 SDP
        self.bn = nn.BatchNorm2d(c2)
        self.sl = nn.SiLU()
        self.eca = ECA(c2)
        
    def forward(self, x):
        output = self.eca(self.sl(self.bn(self.conv(x))))
        #print("# Output Final Size: ", output.size())       # B x C1 x H x W
        return output  
...
class Contigous_Att(nn.Module):    # Input CxHxW
    def __init__(self, c1, c2):
        ...
        self.conv = nn.Conv2d(c1*5, c2, 1, 1)   # after concate 3 SDP
        self.bn = nn.BatchNorm2d(c2)
        self.sl = nn.SiLU()
